...
def get_chart_data(print_on_terminal = False):
    mode_depth = retry_call(get_mode_depth)
    if print_on_terminal: print(f"{mode_depth = }\n")

    positions_actions = retry_call(get_positions_actions)
    if print_on_terminal: print(f"{positions_actions = }\n")

    pot_odds_and_stacks = retry_call(get_pot_odds_and_stacks)
    if print_on_terminal: print(f"{pot_odds_and_stacks = }\n")

    actions_frequencies = retry_call(get_actions_frequencies)
    if print_on_terminal: print(f"{actions_frequencies = }\n")

    combos_dict = get_combos_chart_ev_for_hand(actions_frequencies)
    if print_on_terminal: print(f"{combos_dict = }\n")

    return mode_depth, positions_actions, pot_odds_and_stacks, actions_frequencies, combos_dict


def get_mode_depth():
    soup = make_soup("menu_solutions")

    game_type = soup.find("div", attrs={"data-tst": "gmf_gametype_name"}).get_text(strip=True)
    depth = normalize_float(soup.find("div", class_="gmfover_title_text_depth").get_text(strip=True).split(' ')[1].rstrip('bb'))
    chip_mode = soup.find("span", class_="gmfover_item_text").get_text(strip=True)

    return game_type, chip_mode, depth


def get_positions_actions():
    soup = make_soup("pos_act")

    results = ()
    chunks = soup.find_all("div", class_="hspot-card")
    for chunk in chunks:
        active_action = None
        if "hspotcrd_active" in chunk.get("class", []):
            active_action = "spot"
        positions_chunks = chunk.find_all("div", class_="hspotcrd_inner")
        for position_chunk in positions_chunks:
            position_istack = position_chunk.find_next("div", class_="v-d-flex").get_text().lstrip()
            position = position_istack.split(' ')[0]
            istack = position_istack.split(' ')[1]

            actions_div = position_chunk.find_next("div", class_="hspotcrd_actions")
            actions = actions_div.find_all("div", class_="hspotcrd_action_text")
            actions_text = tuple(action.get_text(strip=True) for action in actions[::-1])

            if not active_action:
                try:
                    active_action = position_chunk.find("div", class_="hspotcrd_action_active").get_text().strip()
                except:
                    # print("no active actions for", position)
                    active_action = 'N/A'

            results += ((position, normalize_float(float(istack)), active_action, actions_text), )

    return results


def get_pot_odds_and_stacks():
    soup = make_soup("pot_odds")

    results = ()

    current_pot_ref = soup.find("div", class_="stt_pot_table")
    current_pot = current_pot_ref.get_text(strip=True).split(" ")[0]

    initial_pot_ref = current_pot_ref.find_next("div")
    initial_pot = initial_pot_ref.get_text(strip=True).split(" ")[0]

    pot_odds_text = initial_pot_ref.find_next("div").get_text()
    pot_odds = pot_odds_text.split(":")[1].split("%")[0]
    if pot_odds.startswith("-"):
        pot_odds = 0

    players_dict = {}
    players = soup.find_all("div", attrs={"data-tst": "player_square"})
    for player in players:
        position = player.find("span", class_="player_name_position").get_text(strip=True)
        stack = player.find("span", class_="player_stack").get_text(strip=True)

        players_dict[position] = normalize_float(float(stack))

    results += ({"initial_pot": normalize_float(float(initial_pot)), "current_pot": normalize_float(float(current_pot)), "pot_odds": normalize_float(float(pot_odds))}, players_dict)

    return results


def get_actions_frequencies():
    soup = make_soup("act_freq_cmb")

    results = ()
    actions = soup.find_all("div", class_="sab_btn")
    for action in actions:
        style = action.find("div", class_="sab_btn_back")["style"].split(":")[1].strip(" ;")
        action_text_parts = action.get_text(",", strip=True).split(",")
        action_name = action_text_parts[0]
        action_frequency = normalize_float(float(action_text_parts[1].rstrip("%")))
        action_combos = normalize_float(float(action_text_parts[2]))
        results += ((action_name, action_frequency, action_combos, style), )

    return results


def get_combos_chart_ev_for_hand(actions_frequencies, ret_hand_res = False, max_attempts = 5, delay = 0.1):
    change_to_range("Strategy + EV")

    positions = ["UTG", "UTG1", "LJ", "HJ", "CO", "BTN", "SB", "BB"]
    spot_position = None
    atm = 0
    while not spot_position and atm < max_attempts:
        if atm > 0:
            print(f"get_combos_chart_ev_for_hand CHART CHECK FAILED {atm} time(s)")
        soup = make_soup("chart")

        try:
            span_text = soup.find("span").get_text(strip=True)
            if span_text in positions:
                spot_position = span_text
        except:
            pass
        atm += 1
        pg.sleep(delay)
        if atm == 5:
            print("get_combos_chart_ev_for_hand FAILED")
            pg.hotkey("alt", "tab")
            sys.exit()
    
    active_combos = []

    combos_chunks = soup.find_all("div", class_="rtc_strategy_ev_range_normalized")

    for combo_chunk in combos_chunks:
        ev_span = combo_chunk.select_one(".rtc_value span")

        if ev_span:
            combo = combo_chunk.find("div", class_="rtc_title").get_text(strip=True)
    ...
    for i in range(matrix_size):
        for j in range(matrix_size):
            combo_ref = combos_matrix[i][j]
            results_per_hand.setdefault(combo_ref, 0)
            results_per_combo.setdefault(combo_ref, 0)
            if combo_ref in active_combos:
                if not ret_hand_res:
                    results_per_combo[combo_ref] = {}
                else:
                    results_per_hand[combo_ref] = {}

                click_x = click_dict["AA"][0] + (j * click_dict["x_offset"])
                click_y = click_dict["AA"][1] + (i * click_dict["y_offset"])
                ...
                paradigm = combo_ref[0] + combo_ref[1]
                test = None
                while test != paradigm and atm < max_attempts:
                    if atm > 0:
                        print(f"get_combos_chart_hands_ev FAILED {atm} time(s) for combo {combo_ref}")

                    soup = make_soup("hands_box")

                    try:
                        test1_div = soup.find("div", class_="cardsymbols_value")
                        test1 = test1_div.get_text()
                        test2 = test1_div.find_next("div", class_="cardsymbols_value").get_text()
                        test = test1 + test2
                    except:
                        pass
                    atm += 1
                    pg.sleep(delay)
                    if atm == 5:
                        print(f"get_combos_chart_hands_ev FAILED for combo {combo_ref}")
                        pg.hotkey("alt", "tab")
                        sys.exit()

                hands_dict = {}
                hands_chunks = soup.find_all("div", class_="htc htc_maximizible study-combos_cell")
                if not hands_chunks:
                    results_per_hand[combo_ref] = 0
                    results_per_combo[combo_ref] = 0

                    continue
                for hand_chunk in hands_chunks:
                    hand = ''

                    cards_chunks = hand_chunk.find_all("div", class_="cardsymbols_value")
                    for card_chunk in cards_chunks:
                        value = card_chunk.get_text(strip=True)

                        suit_class = next(c for c in card_chunk.get("class", []) if c.startswith("color-"))
                        suit = suit_class.split("-")[1]

                        card = value + suit

                        hand += card
                    actions_freq_dict = {}
                    actions_colors_frequencies_chunks = hand_chunk.find_all("div", class_="htc_graph_item")
                    for action_color_frequencie_chunk in actions_colors_frequencies_chunks:
                        style = action_color_frequencie_chunk.get("style", "")
                        freq = normalize_float(round(float(re.search(r'width:\s*([^;]+);', style).group(1).rstrip("%")), 1))
                        act = get_action_name_by_rgb(actions_frequencies, re.search(r'background:\s*([^;]+);', style).group(1))
                        actions_freq_dict[act] = freq
                    ev_max = 0
                    actions_ev_dict = {}
                    actions_evs_chunks = hand_chunk.find_all("div", class_="htc_graph_legend_item_action_name")
                    for action_ev_chunk in actions_evs_chunks:
                        act = action_ev_chunk.get_text().strip()
                        ev = normalize_float(float(action_ev_chunk.find_next("div", class_="htc_graph_legend_item_action_value").get_text()))
                        if ev > ev_max:
                            ev_max = ev
                        actions_ev_dict[act] = ev
                    hand_actions_dict = {}
                    for k, v in actions_freq_dict.items():
                        hand_actions_dict[k] = [v, actions_ev_dict[k]]

                    hands_dict[hand] = hand_actions_dict

                    if not ret_hand_res:
                        results_per_combo[combo_ref] = ev_max, hand_actions_dict

                        break

                if ret_hand_res:
                    results_per_hand[combo_ref] = ev_max, hands_dict

                pg.click(click_x, click_y)
                pg.sleep(0.1)

    if not ret_hand_res:
        return results_per_combo

    return results_per_hand
...
def parse_data(mode_depth, positions_actions, pot_odds_and_stacks, actions_frequencies, combos_dict):
        ...
        for position, action in data.items():
            if action == "spot":
                ...
                if action.startswith("R"):
                    ...
                    # print(f"{position_actions_tuple = }")
                    # print(spot_raise_actions, action)
    ...
    # print(f"{spot_sequency = }")
    # print(f"{spot_actions_text_colors = }")
    # print(f"{original_spot_position = }")
    # print(f"{spot_position = }")
    # print(f"{spot_raise_actions = }")
    ...
    sorted_combos = dict(sorted(combos_dict.items(), key=lambda item: sort_key(item, spot_most_frequent_nonfold_actions, vs), reverse=True))

    combos_order = list(dict(sorted_combos).keys())
    # print(f"{combos_order = }")
    ...
    for combo, data in combos_dict.items():
        ...
        # print(f"{combo_most_frequent_actions = }")
        # print(f"{combo_most_frequent_non_fold_actions = }")
        colors_info = []
        has_play_freq = False
        has_ev = False
...
def sort_key(item: tuple[str, tuple[float, dict[str, list[float]]]], spot_most_frequent_nonfold_actions: list[str], vs: int):
    ...
    actions_dict = value_tuple[1]

    any_ev_max = float("-inf")
    for action in spot_most_frequent_nonfold_actions:
        frequency, ev = actions_dict[action]
        if ev > any_ev_max:
            any_ev_max = ev
...
def unfold_file_blocks(content):
    total_blocks = 0
...
def save_file(file_name, **kwargs):
    if "test" not in file_name:
        with open(f"text_results/{file_name}.txt", "a", encoding="utf-8") as file:
            for key, value in kwargs.items():
                file.write(f"{key} = {value}\n")
            file.write("*" * 50 + "\n")
        with open(f"current.txt", "w", encoding="utf-8") as file:
            for key, value in kwargs.items():
                file.write(f"{key} = {value}\n")
            file.write("*" * 50 + "\n")
    else:
...
def get_action_name_by_rgb(actions_tuples, rgb):
    for action_tuple in actions_tuples:
        action, _, _, l_rgb = action_tuple
        if l_rgb == rgb:
...
def right_click(element: str):
    pg.rightClick(click_dict[element])


def make_soup(element: str) -> bs:
    right_click(element)
    pg.sleep(0.1)
...
def retry_call(func, max_attempts=5, delay=0.1, *args, **kwargs):
    attempt = 1
    ...
    while attempt <= max_attempts:
        try:
            ...
            print(f"Attempt {attempt} failed >>{func.__name__}{args} {kwargs}<<")

        except Exception as error:
            print(f"Error on attempt {attempt} >>{func.__name__}{args} {kwargs}<<: {error}")
    ...
    print(f"All attempts failed >>{func.__name__}{args} {kwargs}<<")
    pg.hotkey("alt", "tab")
    sys.exit()


def get_range_view_type_name():
    soup = make_soup("range_view_name")
    range_view_type = soup.find("div", attrs={"id": "strategy_tab"}).get_text(strip=True)

    return range_view_type


def change_to_range(goal_range):
    range_view_type_name = retry_call(get_range_view_type_name)

    if range_view_type_name == goal_range:

        return

    selector = click_dict["range_view_selector"][range_view_type_name]
    pg.click(selector)

    pg.click(click_dict[goal_range])
...
click_dict = coordinates[resolution]
...
vs_dict = {'UTG': 7, 'UTG1': 6, 'LJ': 5, 'HJ': 4, 'CO': 3, 'BTN': 2, 'SB': 1, 'BB': 1}
...
def main() -> None:
    # 160: [LJ raise-shove, HJ raise-raise-low], 130: [SB call-raise/call-shove], 60: [SB raise-shove vs LJ]
    file_name = "test"
    ...
    mode_depth, positions_actions, pot_odds_and_stacks, actions_frequencies, combos_dict = get_chart_data(True)

    save_file(file_name, mode_depth=mode_depth, positions_actions=positions_actions, pot_odds_and_stacks=pot_odds_and_stacks, actions_frequencies=actions_frequencies, combos_dict=combos_dict)

    content = load_file(file_name)

    chart = unfold_file_blocks(content)

    chart.show()
    chart.save("current.png")
