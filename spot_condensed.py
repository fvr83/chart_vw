...
def get_possible_position_hu_preflop_spots(hero_position: str, positions: list[str]) -> list[str]:
    possible_position_hu_spots = []
    idx_hero = positions.index(hero_position)
    for position in positions:
        idx_position = positions.index(position)
        if idx_hero < idx_position:
            possible_position_hu_spots.append(f'{hero_position}_vs_{position}_raise_nai')
            possible_position_hu_spots.append(f'{hero_position}_vs_{position}_raise_ai')
            possible_position_hu_spots.append(f'{hero_position}_vs_{position}_3bet_nai')
            possible_position_hu_spots.append(f'{hero_position}_vs_{position}_3bet_ai')
        elif idx_hero > idx_position:
            possible_position_hu_spots.append(f'{hero_position}_vs_{position}_limp')
            possible_position_hu_spots.append(f'{hero_position}_vs_{position}_open_raise')
            possible_position_hu_spots.append(f'{hero_position}_vs_{position}_open_shove')
        elif idx_hero == idx_position:
            possible_position_hu_spots.append(f'{hero_position}_rfi')

    return possible_position_hu_spots
...
def get_info_from_spot(gap: int, tier:int , game_mode: str, chip_mode: str, depth: int, hero: str, spot_label: str, villain: str, *info: str) -> tuple | None:
    ...
    for path in Path(origin_folder).iterdir():
        ...
        for block in blocks:
            ...
            if not spot_actions_label and villain_position == None:
...
def get_condensed_range(hero: str, villain: str, spot_action: str, depths: list[int], raise_size='low'):
    print('\n************** START <get_condensed_range> **************')
    temp_combos_dict = {}
    temp_actions_dict = {}

    if 'raise_nai' in spot_action or '3bet_nai' in spot_action or 'vs_rfi' in spot_action:
        spot_label = spot_action + '_' + raise_size
    else:
        spot_label = spot_action

    for depth in depths:
        print(depth)
        actions_frequencies, combos_dict = get_info_from_spot(0, 0, 'MTT', 'ChipEV', depth, hero, spot_label, villain, 'actions_frequencies', 'combos_dict')

        for (spot_act, spot_act_frequency, spot_act_played_combos, _) in actions_frequencies:
            spot_act = 'Allin' if spot_act.startswith('A') else 'Call' if spot_act.startswith('C') else 'Raise' if spot_act.startswith('R') else 'Fold'
            temp_actions_dict.setdefault(spot_act, [0.0, 0.0, 0])
            temp_actions_dict[spot_act][0] += spot_act_frequency
            temp_actions_dict[spot_act][1] += spot_act_played_combos
            temp_actions_dict[spot_act][2] += 1

        for combo, data in combos_dict.items():
            if not data:

                continue
            temp_combos_dict.setdefault(combo, {})
            combo_acions_dict = data[1]
            for action, (fr,ev) in combo_acions_dict.items():
                action = 'Allin' if action.startswith('A') else 'Call' if action.startswith('C') else 'Raise' if action.startswith('R') else 'Fold'
                temp_combos_dict[combo].setdefault(action, [0.0, 0.0, 0])
                temp_combos_dict[combo][action][0] += fr
                temp_combos_dict[combo][action][1] += ev
                temp_combos_dict[combo][action][2] += 1

    condensed_actions_dict = {}
    for k, v in temp_actions_dict.items():
        freq = v[0]
        play_comb = v[1]
        divisor = v[2]
        condensed_actions_dict[k] = [normalize_float(freq/divisor), normalize_float(play_comb/divisor, 2)]
    condensed_actions_dict = dict(sorted(condensed_actions_dict.items(), key=lambda item: item[1][0], reverse=True))

    condensed_combos_dict = {}
    for k, v in temp_combos_dict.items():
        condensed_combos_dict.setdefault(k, {})
        for in_k, in_v in v.items():
            condensed_combos_dict[k].setdefault(in_k, [])
            act_fr = in_v[0]
            act_ev = in_v[1]
            act_div = in_v[2]
            condensed_combos_dict[k][in_k] = [normalize_float(act_fr/act_div), normalize_float(act_ev/act_div)]
    print('************** END <get_condensed_range> **************')
    return spot_label, condensed_actions_dict, condensed_combos_dict


def parse_condensed_data(condensed_actions_dict, condensed_combos_dict, hero: str, villain: str, spot_label: str, depths: list[int], print_info = False):
    print('************** START <parse_condensed_data> **************')
    ...
    vs = vs_dict[hero] if spot_label.startswith('rfi') else 1

    print(f'{condensed_actions_dict = }')

    spot_min_freq_action = min(v[0] for k, v in condensed_actions_dict.items() if k != 'Fold' and v[0] >= 0.5)

    most_frequent_actions = list(condensed_actions_dict.keys())
    print(f'{most_frequent_actions = }')
    action_chain_list = []
    for k, v in condensed_actions_dict.items():
        if k.startswith('R') and hero in alt_positions_list:
            text_color = alt_text_color
        color = color_data[hero][1] if k.startswith('C') else color_data[hero][2] if k.startswith('R') else color_data[hero][6] if k.startswith('A') else color_data[hero][0]
        action_chain_list.append([k, v[0], round(v[0]/spot_min_freq_action)])
    print(f'{action_chain_list = }')

    
    # title_spot_actions_colors_list = [f'{depths[0]}-{depths[-1]}', def_text_color, alt_text_color], actions_chain_list[0] + [def_text_color if hero_position not in alt_positions_list else alt_text_color] + [color_data[hero_position][2]]]
    ...
    #         if act.startswith('R'):
    #             raise_size, color = get_spot_raise_size_and_color(hero_position, act, actions_frequencies)
    #             if raise_size not in ['low', 'low-med']:
    #                 title_position_text_color = alt_text_color
    ...
    #         if action.startswith('R'):
    #             raise_size, _ = get_spot_raise_size_and_color(hero_position, action, actions_frequencies)
    ...
    print('************** END <parse_condensed_data> **************')
...
def main() -> None:
    ...
    depths_200_100 = [200, 160, 130, 100] # +0.8
    depths_80_45 = [80, 70, 60, 55, 50, 45] # +0.3
    depths_40_32 = [40, 38 ,35, 32] # +0.3
    depths_30_25 = [30, 28 ,26, 25] # +0.4
    ## 22
    depths_22_19 = [22, 20, 19] # +0.9
    # 17
    depths_17_16 = [17, 16] # +0.5
    depths_15_13 = [15, 14, 13] # +0.4
    depths_12_10 = [12, 11, 10] # +0.6
    depths_9_7 = [9, 8, 7] # +0.7

    depths_pool = [depths_200_100, depths_80_45, depths_40_32, depths_30_25, depths_22_19, depths_17_16, depths_15_13, depths_12_10, depths_9_7]
    hero = 'UTG1'
    spot_action = 'vs_rfi'
    villain =  'UTG'

    for depth_list in depths_pool:
        spot_label, condensed_actions_dict, condensed_combos_dict = get_condensed_range(hero, villain, spot_action, depth_list)
        parse_condensed_data(condensed_actions_dict, condensed_combos_dict, hero, villain, spot_label, depth_list)
        print('\n************** <main> debug **************')
        print(f'\n{condensed_actions_dict = }')
        # print(f'\n{condensed_combos_dict = }')
