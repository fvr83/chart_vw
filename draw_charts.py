...
def parse_data(mode_depth, positions_actions, pot_odds_and_stacks, actions_frequencies, combos_dict):
    ...
    spot_raise_actions = []
    spot_actions_taken = []
    raise_sizes_options = ["low", "low_med", "med", "med_high", "high"]
    raise_size = ""
    ...
    for index, positon_action_tuple in enumerate(positions_actions):
        ...
        if action_choosed not in ["Fold", "N/A"]:
            spot_actions_taken.append(action_choosed)
        ...
        if action_choosed not in ["N/A", "Fold"]:
            ...
            if "Raise" in action_choosed:
                ...
                raise_index = raise_actions.index(action_choosed)
                raise_size = raise_sizes_options[raise_index]
    ...
    return combo_colors_info_dict, spot_actions_text_colors, combos_order, fold_combos_final, spot_position, spot_actions_taken_label, spot_string
...
origin_folder = "text_results"
destination_folder = "img_results" # final_folder
...
for path in Path(origin_folder).iterdir():
    ...
    for block_index, block in enumerate(blocks):
        ...
        combo_colors_info_dict, spot_actions_text_colors, combos_order, fold_combos, spot_position, spot_actions_taken_label, spot_string = parse_data(mode_depth, positions_actions, pot_odds_and_stacks, actions_frequencies, combos_dict)
        chart = draw_proof_chart(combo_colors_info_dict, spot_actions_text_colors, combos_order, fold_combos)
        chart_type = "ranking"
        final_folder = f"{destination_folder}\\{game_mode}\\{chip_mode}\\{gap}_{tier}\\{chart_type}\\{spot_actions_taken_label}\\{depth.removesuffix('bb')}\\{spot_position}"
